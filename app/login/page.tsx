  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#080d19]">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute -top-40 -left-40 h-[600px] w-[600px] rounded-full bg-cyan-500/[0.08] blur-[150px]" />
        <div className="absolute -bottom-40 -right-40 h-[700px] w-[700px] rounded-full bg-violet-500/[0.09] blur-[150px]" />
      ...
      >
        <div className="rounded-2xl px-8 py-10" style={{
          ...
          <div className="mb-8 text-center">
            ...
            <p className="mt-1.5 text-sm text-slate-400">Sign in to your WageWise account</p>
          </div>

          {reason === "expired" && (
            <div className="mb-4 rounded-xl border border-amber-500/20 bg-amber-500/[0.06] px-4 py-3 text-xs text-amber-400">Your session expired. Please log in again.</div>
          )}
          {authError === "auth_callback" && (
            <div className="mb-4 rounded-xl border border-rose-500/20 bg-rose-500/[0.06] px-4 py-3 text-xs text-rose-400">Sign in failed. Please try again.</div>
          )}
          {error && (
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-xl border border-white/[0.06] bg-[#0b1120] px-4 py-3 text-sm text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/40 focus:ring-2 focus:ring-cyan-500/10 focus:shadow-[0_0_15px_rgba(6,182,212,0.08)]" placeholder="Enter your password" />
            </div>
          ...
          <div className="mt-4 text-center">
            <Link href="/login" className="text-xs text-slate-500 transition-colors hover:text-cyan-400">Forgot password?</Link>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl px-6 py-4 text-center" style={{
          ...
          <p className="text-sm text-slate-400">
            ...
            <Link href="/signup" className="font-semibold text-cyan-400 transition-colors hover:text-cyan-300">Sign up</Link>
          </p>
        </div>
      </motion.div>
    </main>
